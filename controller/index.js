...
var Controller = function( Laadpaal, BaseController ){
  ...
  controller.index = function(req, res){
    res.send('This is a NL Laadpaal provider. ');
  };
  ...
  controller.register = function(req, res){
    if ( !req.body.host ){
      ...
      Laadpaal.register( req.body.id, req.body.host, function(err, id){
        if (err) {
  ...
  controller.getData = function(req,res){
	  Laadpaal.find(req.params.id, req.query, function(err, data){
	   if ( err ){
          res.json( err, 500 );
		  return
	   }
